...
const Discovery = () => {
  ...
  return (
    <Stack className="discovery-section">
      <Stack className="container">
        ...
        <Stack className="pagination-numbers" direction="row">
          {Array.from({ length: pageCount }).map((_, index) => (
            <Box
              ...
              className={`page-number ${
                index === safeActivePage ? "active" : ""
              }`}
              onClick={() => setActivePage(index)}
              aria-current={index === safeActivePage ? "page" : undefined}
            >
              {index + 1}
            </Box>
          ))}
